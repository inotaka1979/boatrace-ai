...
BW = "biwako_modules"
# platform "suminoe_yoso": 住之江型。SP は iframe で per-race htm を読む。直前情報
...
def parse_suminoe_yoso(html, sid, rno):
    """住之江型(yoso05{RR}.htm 直前情報予想)→ race dict | None。

    住之江は時刻列の th が全て同じ class(col10)で col 駆動が効かず、枠セルが
    waku01..waku06。ヘッダ th の並び順(枠/選手名/体重/チルト/展示/一周/まわり足)
    から位置を取り、td を位置で読む(直線列は無い)。
    """
    soup = BeautifulSoup(html, "html.parser")
    target = None
    for tbl in soup.find_all("table"):
    ...
    def _idx(label):
        return head.index(label) if label in head else -1
    ...
    for tr in target.find_all("tr"):
        ...
        if not first:
            continue
        m = re.search(r"waku0*(\d+)", " ".join(first.get("class") or []))
        if not m:
            continue
        waku = int(m.group(1))
        if not (1 <= waku <= 6):
...
def scrape_suminoe_yoso(base, jcd, date_str):
    ...
    out = []
    for rno in range(1, 13):
        url = f"{base}/asp/kyogi/{jcd}/sp/yoso05{rno:02d}.htm"
        try:
            html = fetch_text(url, timeout=12, retries=1,
                              headers={"Referer": base + "/sp/"})
        except Exception:
