...
# Phase 2 完遂続編 (Clearwing): rest からさらに sub-chunk を 2 つ分離。
#   stats: 成績タブ / バックテストタブを開いた時のみ
#   detail: レース詳細ページを開いた時のみ
OUT_REST_STATS = ROOT / "assets" / "app-rest-stats.js"
OUT_REST_DETAIL = ROOT / "assets" / "app-rest-detail.js"
...
#   成績タブ / バックテストタブを開いた時のみ download / parse する。
...
# 同じく lazy: レース詳細ページ専用 chunk (openRace を含む 4 bundle)。
#   ユーザが場一覧 → レースをタップした時に動的 load。
#   ~35KB minified の初回 download 削減 (app-rest.min.js から分離)。
REST_DETAIL_BUILD_MARKERS = {
    'REPORTING_RACE_DETAIL',             # openRace orchestrator + header + weather + result + exhibition
    'REPORTING_RACE_DETAIL_BOATS',       # 出走表
    'REPORTING_RACE_DETAIL_PREDICTION',  # 番組予想 + 直前予想
    'REPORTING_RACE_DETAIL_BETS',        # 買い目 + 穴予想
}
# rest_parts から detail chunk へ移す関数 (現状なし、必要に応じて追加)
REST_DETAIL_FUNCTION_NAMES: set[str] = set()
...
for marker in sorted(REST_ONLY_BUILD_MARKERS):
    ...
    # bundle を 3 種類のどれに振り分けるか
    if marker in REST_STATS_BUILD_MARKERS:
        rest_stats_bundles.append(block)
    elif marker in REST_DETAIL_BUILD_MARKERS:
        rest_detail_bundles.append(block)
    else:
        rest_bundle_extracted.append(block)
    src_remaining = src_remaining[:s] + src_remaining[e_end + 1 :]

# rest_parts から REST_STATS_FUNCTION_NAMES / REST_DETAIL_FUNCTION_NAMES に該当する
# 関数を抜き出して各 chunk に振り分け
rest_parts_stats: list[str] = []
rest_parts_detail: list[str] = []
rest_parts_filtered: list[str] = []
for body in rest_parts:
    # 関数名を取得 (body は `function NAME(...) {...}` で始まる)
    m = re.match(r'^(?:async\s+)?function\s+(\w+)\s*\(', body)
    name = m.group(1) if m else ''
    if name in REST_STATS_FUNCTION_NAMES:
        rest_parts_stats.append(body)
    elif name in REST_DETAIL_FUNCTION_NAMES:
        rest_parts_detail.append(body)
    else:
        rest_parts_filtered.append(body)
rest_parts = rest_parts_filtered
...
# Phase 2 完遂続編 (Clearwing): rest-stats sub-chunk。
#   成績タブ / バックテストタブを開いた時のみ動的 load。
rest_stats_out = (
    ...
    "'use strict';\n"
    "\n"
    + ("\n\n".join(rest_stats_bundles) + "\n\n" if rest_stats_bundles else "")
    + "\n\n".join(rest_parts_stats)
    + "\n"
)

# Phase 2 完遂続編: rest-detail sub-chunk。
#   レース詳細ページを開いた時のみ動的 load。
#   openRace + boats + prediction + bets の 4 bundle (~35KB minified) を含む。
rest_detail_out = (
    "// (auto-generated by scripts/split_app.py)\n"
    "// 編集禁止 — assets/app.js を編集して `python3 scripts/split_app.py` を実行\n"
    ...
    "'use strict';\n"
    "\n"
    + ("\n\n".join(rest_detail_bundles) + "\n\n" if rest_detail_bundles else "")
    + "\n\n".join(rest_parts_detail)
    + "\n"
)

OUT_CRITICAL.write_text(critical_out)
OUT_REST.write_text(rest_out)
OUT_REST_STATS.write_text(rest_stats_out)
OUT_REST_DETAIL.write_text(rest_detail_out)

print()
print(f'wrote {OUT_CRITICAL}    ({len(critical_out):,} chars)')
print(f'wrote {OUT_REST}         ({len(rest_out):,} chars)')
print(f'wrote {OUT_REST_STATS}   ({len(rest_stats_out):,} chars)')
print(f'wrote {OUT_REST_DETAIL}  ({len(rest_detail_out):,} chars)')
