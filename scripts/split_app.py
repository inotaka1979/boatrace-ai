...
# Phase 2 完遂続編 (Clearwing): rest からさらに stats/backtest 用 sub-chunk を分離。
#   ユーザが成績タブ / バックテストタブを開くまで download / parse しない。
...
rest_stats_bundles: list[str] = []   # Phase 2 完遂続編: lazy sub-chunk
for marker in REST_ONLY_BUILD_MARKERS:
    ...
    # REST_STATS_BUILD_MARKERS に該当する bundle は app-rest-stats.js に振り分け
...
# rest_parts から REST_STATS_FUNCTION_NAMES に該当する関数を抜き出して stats chunk へ
...
for body in rest_parts:
    ...
    if m and m.group(1) in REST_STATS_FUNCTION_NAMES:
