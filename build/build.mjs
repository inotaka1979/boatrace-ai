...
async function main() {
  ...
  // Phase 2 完遂続編: app-rest-stats.js (lazy sub-chunk) も同様に minify
  ...
  const BUDGETS = [
    ...
    { path: 'assets/app-rest.min.js',     max: 120000, level: 'warn' },   // sub-chunk 分離後の縮小予算
  ...
  try {
    ...
    // Phase 2 完遂続編: app-rest-stats.min.js も hash 計算に含めて lazy chunk 更新を検知
    const restStatsBuf = await readFile(resolve(ROOT, 'assets/app-rest-stats.min.js')).catch(() => Buffer.alloc(0));
    const combined = Buffer.concat([criticalBuf, restBuf, restStatsBuf]);
