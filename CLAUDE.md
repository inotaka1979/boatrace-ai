...
## 修正履歴 (2026-07-22: 日別成績の「飛び飛び」欠落を archive 遡及補完で解消)

日別成績 (#pageDaily) が毎日開催されているのに日ごと飛び飛びに欠落する不具合を修正。

### 原因
予想 entry は「その日にアプリを開いた」時点で作られるが、`actual`（確定着順）は
その場の `resultData`（＝当日分のみ）からしか埋まらない。夜のレースが確定する前に
アプリを閉じた日などは `actual=null` のまま残り、`calcDailyStats` の `!h.actual`
フィルタでその日が丸ごと欠落していた（＝飛び飛び）。

### 修正 (src/reporting/daily_stats_page.js — rest-stats chunk)
- `_backfillDailyActualsFromArchive()` を新設。過去日で `actual` 未取得の entry を
  results archive (`/results/v2/YYYY/YYYYMMDD.json`) から遡及補完する。
  - 直近 14 日・欠測日のみ・500ms 間隔・セッション 1 回に制限。
  - 既存 actual は上書きせず、`race_date` 一致チェックで前日結果汚染を防止。
  - 今日分は従来の live 経路 (savePrediction / updateHistoryWithResults) が担当。
- `_applyHitAndPayout()` で checkHit + payout 補完を self-contained に再実装
  （checkHit は別 bundle のため rest-stats から確実に呼べるようインライン化）。
- `renderDailyStats()` 冒頭でバックグラウンド起動し、補完できたら 1 度だけ再描画。
- critical 予算は不変（本 chunk は rest-stats、日別タブを開いた時のみ動作）。

### 限界
その日に一度もアプリを開かず予想 entry 自体が無い日は復活できない（後付け予想は
リーク・整合性の観点で行わない）。アプリを開いた日の取りこぼしのみ補完する。

### 検証
- 45/45 テスト PASS、build:check EXIT=0、tsc/eslint 0 errors、prettier 準拠。
- 実データで actual 補完・的中判定・払戻補完（3連単¥3,390 / 2連単¥1,740）を確認。
